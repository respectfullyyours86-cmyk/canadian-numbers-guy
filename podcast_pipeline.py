...
Live data → Groq script → Edge TTS audio → Cloudflare R2 → Podcasting 2.0 RSS
"""
...
ARTWORK_URL          = "https://pub-6b50685205f14f8d8838ec07c7deb217.r2.dev/artwork.jpg.png"
...
def fetch_market_data() -> dict:
    """Pull live Canadian economic data from free APIs. No API keys required."""
    data = {}

    # CAD/USD — European Central Bank data via Frankfurter (very reliable)
    try:
        resp = requests.get(
            "https://api.frankfurter.app/latest?from=USD&to=CAD", timeout=10
        )
        cad_per_usd = resp.json()["rates"]["CAD"]
        data["cad_per_usd"] = round(cad_per_usd, 4)
        data["usd_per_cad"] = round(1 / cad_per_usd, 4)
    except Exception:
        pass

    # WTI Crude Oil (USD/barrel) — biggest driver of Canadian gas prices
    try:
        resp = requests.get(
            "https://query1.finance.yahoo.com/v8/finance/chart/CL=F",
            headers=YF_HEADERS, timeout=10
        )
        data["oil_wti_usd"] = round(
            resp.json()["chart"]["result"][0]["meta"]["regularMarketPrice"], 2
        )
    except Exception:
        pass

    # Natural Gas (USD/MMBtu) — home heating costs
    try:
        resp = requests.get(
            "https://query1.finance.yahoo.com/v8/finance/chart/NG=F",
            headers=YF_HEADERS, timeout=10
        )
        data["natural_gas_usd"] = round(
            resp.json()["chart"]["result"][0]["meta"]["regularMarketPrice"], 2
        )
    except Exception:
        pass

    # Wheat futures (USD/bushel) — grocery/food price indicator
    try:
        resp = requests.get(
            "https://query1.finance.yahoo.com/v8/finance/chart/ZW=F",
            headers=YF_HEADERS, timeout=10
        )
        data["wheat_usd"] = round(
            resp.json()["chart"]["result"][0]["meta"]["regularMarketPrice"], 2
        )
    except Exception:
        pass
    ...
    try:
        resp = requests.get(
            "https://www.bankofcanada.ca/valet/observations/AUCAL_OVERNIGHT/json?recent=1",
            timeout=10
        )
        boc = resp.json()
        data["boc_rate"] = float(boc["observations"][0]["AUCAL_OVERNIGHT"]["v"])
    except Exception:
        pass

    # Estimate Canadian pump price from crude oil
    # Formula: crude per litre in CAD + taxes/refining/retail margin (~$0.78)
    if "oil_wti_usd" in data and "cad_per_usd" in data:
        crude_cad_litre = (data["oil_wti_usd"] / 158.987) * data["cad_per_usd"]
        data["est_gas_cad_litre"] = round(crude_cad_litre + 0.78, 2)

    return data


def format_market_data(data: dict) -> str:
    """Format fetched data into a readable block for the Groq prompt."""
    if not data:
        return ""
    lines = ["LIVE MARKET DATA FOR TODAY'S EPISODE:"]
    if "cad_per_usd" in data:
        lines.append(f"- CAD/USD: 1 USD = {data['cad_per_usd']} CAD")
    if "boc_rate" in data:
        lines.append(f"- Bank of Canada Overnight Rate: {data['boc_rate']}%")
    if "oil_wti_usd" in data:
        lines.append(f"- WTI Crude Oil: ${data['oil_wti_usd']} USD/barrel")
    if "est_gas_cad_litre" in data:
        lines.append(f"- Estimated Canadian pump price: ~${data['est_gas_cad_litre']} CAD/litre")
    if "natural_gas_usd" in data:
        lines.append(f"- Natural Gas: ${data['natural_gas_usd']} USD/MMBtu (home heating)")
    if "wheat_usd" in data:
        lines.append(f"- Wheat futures: ${data['wheat_usd']} USD/bushel (food cost indicator)")
    return "\n".join(lines)
...
def generate_script(topic: str) -> str:
    ...
    print(f"        {len(market_data)} data points fetched")
...
Use those real numbers naturally throughout the script wherever relevant — don't just list them, weave them into the conversation so they feel like proof points, not a data dump.

Structure it exactly like this:

Open with a warm good morning greeting welcoming the listener to The Canadian Numbers Guy. Keep it natural and brief — one or two sentences. Something like "Good morning and welcome to The Canadian Numbers Guy — the show where we cut through the noise and tell you what the numbers actually mean for your life."

Then cover three angles:
1. Everyday Canadians — renters, workers, people living paycheque to paycheque. Use the real price data to make it concrete. What does this mean at the gas pump, the grocery store, the hydro bill?
2. The investor class — TFSA holders, landlords, Bay Street types. How are they positioned differently by these same numbers?
3. The raw data takeaway — what one or two numbers from today tell the clearest story about where Canada is headed.

Close with a short warm sign-off — something like "That's the data for today. Make of it what you will. I'll see you tomorrow morning. Take care."
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        try:
            ...
            topic = (
                "Canadian inflation and what rising energy and food costs "
                "mean for everyday Canadians, investors, and the Bank of Canada."
            )
